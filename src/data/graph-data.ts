import { MetricProps } from "@/app/trade/[id]/GraphDashboard/Metric";

export interface GraphData {
  name: number;
  Implied: number;
  Underlying: number;
}

export const apyGraphInitialData: GraphData[] = [
  { name: 1, Implied: 50, Underlying: 100 },
  { name: 2, Implied: 37, Underlying: 120 },
  { name: 3, Implied: 45, Underlying: 150 },
  { name: 4, Implied: 55, Underlying: 180 },
  { name: 5, Implied: 59, Underlying: 200 },
  { name: 6, Implied: 63, Underlying: 499 },
  { name: 7, Implied: 53, Underlying: 50 },
  { name: 8, Implied: 52, Underlying: 100 },
  { name: 9, Implied: 79, Underlying: 200 },
  { name: 10, Implied: 94, Underlying: 222 },
  { name: 11, Implied: 73, Underlying: 210 },
  { name: 12, Implied: 41, Underlying: 300 },
  { name: 13, Implied: 21, Underlying: 50 },
  { name: 14, Implied: 86, Underlying: 190 },
  { name: 15, Implied: 77, Underlying: 300 },
  { name: 16, Implied: 66, Underlying: 400 },
  { name: 17, Implied: 55, Underlying: 200 },
  { name: 18, Implied: 99, Underlying: 50 },
  { name: 19, Implied: 76, Underlying: 100 },
  { name: 20, Implied: 85, Underlying: 100 },
  // Adicione mais dados conforme necessário...
];

export const priceGraphInitalData: GraphData[] = [
  { name: 1, Implied: 50, Underlying: 10 },
  { name: 2, Implied: 37, Underlying: 12 },
  { name: 3, Implied: 45, Underlying: 15 },
  { name: 4, Implied: 55, Underlying: 18 },
  { name: 5, Implied: 59, Underlying: 20 },
  { name: 6, Implied: 63, Underlying: 49 },
  { name: 7, Implied: 53, Underlying: 50 },
  { name: 8, Implied: 52, Underlying: 10 },
  { name: 9, Implied: 79, Underlying: 20 },
  { name: 10, Implied: 94, Underlying: 22 },
  { name: 11, Implied: 73, Underlying: 21 },
  { name: 12, Implied: 41, Underlying: 30 },
  { name: 13, Implied: 21, Underlying: 50 },
  { name: 14, Implied: 86, Underlying: 190 },
  { name: 15, Implied: 77, Underlying: 300 },
  { name: 16, Implied: 66, Underlying: 40 },
  { name: 17, Implied: 55, Underlying: 200 },
  { name: 18, Implied: 99, Underlying: 50 },
  { name: 19, Implied: 76, Underlying: 100 },
  { name: 20, Implied: 85, Underlying: 10 },
]

export type MetricData = MetricProps;

export const metricsData: MetricData[] = [
  {
    title: "Liquidity",
    typeValue: "CURRENCY",
    value: 3255002.15,
    percent: -4.84
  },
  {
    title: "24h Volume",
    typeValue: "CURRENCY",
    value: 2604072.41,
    percent: 537.3
  },
  {
    title: "Underlying APY",
    typeValue: "PERCENT",
    value: 0,
    percent: 0
  },
  {
    title: "Fixed APY",
    typeValue: "PERCENT",
    value: 18.55,
    percent: 25.11
  }
]